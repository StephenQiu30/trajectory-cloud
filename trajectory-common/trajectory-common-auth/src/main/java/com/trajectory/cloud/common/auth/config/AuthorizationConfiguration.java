...
public class AuthorizationConfiguration implements StpInterface {
    ...
    public List<String> getRoleList(Object loginId, String loginType) {
        ...
        if (StrUtil.equals(fromSource, SecurityConstant.INNER)) {
            return Collections.singletonList(UserConstant.ADMIN_ROLE);
        }

        SaSession saSession = StpUtil.getSessionByLoginId(loginId);
        Object user = saSession.get(UserConstant.USER_LOGIN_STATE);
        if (user == null) {
            return new ArrayList<>();
        }
        return Collections.singletonList(JSONUtil.parseObj(user).getStr("userRole"));
    }
